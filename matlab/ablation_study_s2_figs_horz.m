...
font_weight = 'normal';
color_alpha = 0.7;
method_order = ["pg-", "pg", "pg-po", "sg-", "sg", "pg-mpc", "sg-mpc", "sg-mpc-po"];
        ...
        if(stdr_rates(i, j) ~= 0)
            if(j == success_idx)
                ...
                if(xbarRange1(i, j) <= range_buff)
                    if(j == collision_idx)
                        text(xbarTop1(i, j)+move_right_offset, ybarCnt1(i, j), txt1(i, j), ...
...
exportgraphics(gcf,'ab_rates_horz_s2_stdr.png')
end
%% gazebo rate bar
range_buff = 10;
move_left_offset = 10;
...
exportgraphics(gcf,'ab_rates_horz_s2_gazebo.png')
end